...
result = query("How to design a synthesis protocol for a high-entropy Prussian blue analog (HE-PBA) catalyst using cobalt cyanide K3[Co(CN)6] as the framework and incorporating five transition metals (Fe, Co, Ni, Mn, Zn)? Please provide step-by-step instructions.",
    max_iter=50
)


# 将结果输出到txt文件
with open(f"gemini_result/{config2['try_count']}_50.txt", "w", encoding="utf-8") as f:
    f.write(str(result))
