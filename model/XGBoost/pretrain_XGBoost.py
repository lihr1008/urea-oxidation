...
plt.savefig(f'pretrain_XGB.png')
plt.show()

# 保存模型
with open('xgb_premodel.pkl', 'wb') as f:
    pickle.dump(model, f)
...
with pd.ExcelWriter(f'pretrain_XGB_result_all.xlsx') as writer:
...
# 保存标准化器
with open('pretrain_XGB_norm_y.pckl', 'wb') as f:
    pickle.dump(norm_y, f)

with open('pretrain_XGB_norm_x.pckl', 'wb') as f:
