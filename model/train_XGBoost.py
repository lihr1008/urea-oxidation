...
# 使用预训练模型获取特征
predictor = Predictor('xgb_premodel.pkl')
...
plt.savefig('train_XGB.png', dpi=300, bbox_inches='tight')
plt.show()
...
with pd.ExcelWriter('train_XGB_result_all.xlsx') as writer:
...
# 保存标准化器
with open('train_XGB_norm_y.pckl', 'wb') as f:
    pickle.dump(norm_y, f)

with open('train_XGB_norm_x.pckl', 'wb') as f:
