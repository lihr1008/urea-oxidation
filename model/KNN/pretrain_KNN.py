...
plt.savefig(f'pretrain_KNN.png')
plt.show()

# 保存模型
with open('knn_premodel.pkl', 'wb') as f:
    pickle.dump(model, f)
...
with pd.ExcelWriter(f'pretrain_KNN_result_all.xlsx') as writer:
...
# 保存标准化器
with open('pretrain_KNN_norm_y.pckl', 'wb') as f:
    pickle.dump(norm_y, f)

with open('pretrain_KNN_norm_x.pckl', 'wb') as f:
