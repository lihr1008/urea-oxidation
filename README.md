...
`query_select_*.py`: These scripts leverage the DeepSearcher framework together with a vector database (Milvus) to identify element combinations that satisfy multiple catalytic, economic, and environmental constraints. Relevant literature abstracts published over the past decade were retrieved from the Web of Science database using the keywords of urea oxidation and Prussian blue analogues. Each script corresponds to a specific LLM backend (e.g., deepseek-reasoner, Qwen3-Max, ChatGPT-5, or gemini-3-pro-preview), enabling systematic cross-model comparison and validation of the selection results.  
`query_syn_*.py`: These scripts utilize the DeepSearcher framework and the Milvus vector database to recommend feasible synthetic routes based on 93 curated literature sources related to Prussian blue analogues. The same set of advanced LLMs was employed to ensure robustness and consistency in synthesis reasoning, allowing cross-model comparison and reducing model-specific bias in the generated recommendations.
3. NLP    
...
`dimension reduction.py`: Analysis of overpotential distributions of high-entropy Prussian blue materials of different compositions using PCA.  
`model_load.py`: Directly load the saved model to obtain results.
