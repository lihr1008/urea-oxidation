...
class DirectExperimentModel(nn.Module):
    def __init__(self, input_dim=5, hidden_units=512, output_dim=1, dropout=0.4):
        super().__init__()
...
def get_run_dirs_by_id(base_dir, run_id):
    base_dir = Path(base_dir)
    model_dir = base_dir / "model_load" / run_id
    result_dir = base_dir / "results" / run_id
    model_dir.mkdir(parents=True, exist_ok=True)
    result_dir.mkdir(parents=True, exist_ok=True)
    return run_id, model_dir, result_dir
