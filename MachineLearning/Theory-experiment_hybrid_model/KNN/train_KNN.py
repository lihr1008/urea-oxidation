...
def get_default_parameters():
    # K近邻模型的默认参数
    params = {
        'seed': 106, #36,49，5，76,59,162,163,123,178,
        'n_neighbors': 7,        # 邻居数量
...
# 数据标准化
with open(SCRIPT_DIR / 'pretrain_KNN_norm_x.pckl', 'rb') as f:
    pretrain_norm_x = pickle.load(f)
norm_y = StandardScaler().fit(y_train)

# 标准化数据
x_train_ = pretrain_norm_x.transform(x_train)
x_test_ = pretrain_norm_x.transform(x_test)
y_train_ = norm_y.transform(y_train).ravel()  # 转换为1D数组
y_test_ = norm_y.transform(y_test).ravel()    # 转换为1D数组

# 使用预训练模型获取特征
predictor = Predictor(SCRIPT_DIR / 'knn_premodel.pkl')
...
plt.savefig(SCRIPT_DIR / 'train_KNN.png', dpi=300, bbox_inches='tight')
plt.show()
...
with pd.ExcelWriter(SCRIPT_DIR / 'train_KNN_result_all.xlsx') as writer:
...
# 保存模型
with open(SCRIPT_DIR / 'knn_model.pkl', 'wb') as f:
    pickle.dump(model, f)

# 保存标准化器
with open(SCRIPT_DIR / 'train_KNN_norm_y.pckl', 'wb') as f:
...
with pd.ExcelWriter(SCRIPT_DIR / 'space_one_percent.xlsx') as writer:
