...
import pickle
class Predictor:
    def __init__(self, model_path, norm_x_path):
        """初始化预测器"""
        with open(model_path, 'rb') as f:
            self.model = pickle.load(f)

        with open(norm_x_path, 'rb') as f:
            self.norm_x = pickle.load(f)

    def predict(self, x_data):

        # 标准化输入 - 使用特征名称来避免警告
        x_scaled = self.norm_x.transform(x_data)
...
def get_default_parameters():
    # 随机森林模型的默认参数
    params = {
        'seed': 73, #19,37,93,76,150,102
        'n_estimators': 100,  # 树的数量
        'min_samples_split': 2,  # 内部节点再划分所需最小样本数
        'min_samples_leaf': 2,   # 叶子节点最少样本数
        'max_features': 0.8,  # 每次分割时考虑的特征数量
...
# 读取训练数据
excel = DATA_DIR / "experiment.xlsx"
...
# 使用预训练模型获取特征
predictor = Predictor(
    SCRIPT_DIR / 'rf_premodel.pkl',
    SCRIPT_DIR / 'pretrain_RF_norm_x.pckl',
)
train_pred_ = predictor.predict(x_train)
test_pred_ = predictor.predict(x_test)
...
plt.savefig(SCRIPT_DIR / 'train.png', dpi=300, bbox_inches='tight')
plt.show()

# 保存结果到Excel
train_pred_df = pd.DataFrame(train_pred, columns=['overpotential'])
test_pred_df = pd.DataFrame(test_pred, columns=['overpotential'])

with pd.ExcelWriter(SCRIPT_DIR / 'train_result_all.xlsx') as writer:
...
# 保存标准化器
with open(SCRIPT_DIR / 'train_norm_y.pckl', 'wb') as f:
