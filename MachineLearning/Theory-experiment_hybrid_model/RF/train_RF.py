...
# 数据标准化
with open(SCRIPT_DIR / 'pretrain_RF_norm_x.pckl', 'rb') as f:
...
# 使用预训练模型获取特征
predictor = Predictor(SCRIPT_DIR / 'rf_premodel.pkl')
...
plt.savefig(SCRIPT_DIR / 'train_RF.png', dpi=300, bbox_inches='tight')
plt.show()
...
with pd.ExcelWriter(SCRIPT_DIR / 'train_RF_result_all.xlsx') as writer:
...
# 保存标准化器
with open(SCRIPT_DIR / 'train_RF_norm_y.pckl', 'wb') as f:
