...
plt.savefig(SCRIPT_DIR / 'pretrain.png')
plt.show()

# 保存模型
with open(SCRIPT_DIR / 'svr_premodel.pkl', 'wb') as f:
    pickle.dump(model, f)
...
with pd.ExcelWriter(SCRIPT_DIR / 'pretrain_result_all.xlsx') as writer:
...
# 保存标准化器
with open(SCRIPT_DIR / 'pretrain_norm_y.pckl', 'wb') as f:
    pickle.dump(norm_y, f)

with open(SCRIPT_DIR / 'pretrain_norm_x.pckl', 'wb') as f:
