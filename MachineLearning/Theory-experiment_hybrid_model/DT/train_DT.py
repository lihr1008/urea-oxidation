...
# 数据标准化
with open(SCRIPT_DIR / 'pretrain_DT_norm_x.pckl', 'rb') as f:
...
# 使用预训练模型获取特征
predictor = Predictor(SCRIPT_DIR / 'dt_premodel.pkl')
...
plt.savefig(SCRIPT_DIR / 'train_DT.png', dpi=300, bbox_inches='tight')
plt.show()
...
with pd.ExcelWriter(SCRIPT_DIR / 'train_DT_result_all.xlsx') as writer:
...
# 保存模型
with open(SCRIPT_DIR / 'dt_model.pkl', 'wb') as f:
    pickle.dump(model, f)

# 保存标准化器
with open(SCRIPT_DIR / 'train_DT_norm_y.pckl', 'wb') as f:
