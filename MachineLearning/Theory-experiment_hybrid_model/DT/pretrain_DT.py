...
# 创建决策树模型 - 使用MultiOutputRegressor包装以支持多输出回归
...
plt.savefig(SCRIPT_DIR / 'pretrain_DT.png')
plt.show()

# 保存模型
with open(SCRIPT_DIR / 'dt_premodel.pkl', 'wb') as f:
    pickle.dump(model, f)
...
with pd.ExcelWriter(SCRIPT_DIR / 'pretrain_DT_result_all.xlsx') as writer:
...
# 保存标准化器
with open(SCRIPT_DIR / 'pretrain_DT_norm_y.pckl', 'wb') as f:
    pickle.dump(norm_y, f)

with open(SCRIPT_DIR / 'pretrain_DT_norm_x.pckl', 'wb') as f:
