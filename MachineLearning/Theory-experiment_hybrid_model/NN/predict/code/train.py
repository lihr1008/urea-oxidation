...
torch.save(model, RESULT_DIR / 'PBA-ml-train_model.pth')
state_dict = model.state_dict()
torch.save(state_dict, RESULT_DIR / 'train_model_weights.pth')
...
#保存
f = open(RESULT_DIR / 'train_norm_y.pkl', 'wb')
pickle.dump(norm_y, f)
f.close()

import shap
def combined_model(x_np):
    x_norm = pretrain_norm_x.transform(x_np)
...
df_combined.to_excel(RESULT_DIR / "shap_data.xlsx", index=False)
...
with pd.ExcelWriter(RESULT_DIR / 'train_result_all.xlsx') as writer:
