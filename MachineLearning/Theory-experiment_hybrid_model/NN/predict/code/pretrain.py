...
excel = DATA_DIR / "metals_activity.xlsx"
...
optimizer = optim.Adam(premodel.parameters(), lr=1e-3, weight_decay=1e-5)
...
for i in range(y.shape[1]):
    ...
    axes[i, 0].text(y_max, y_min, f'r={train_corr[i]:.4f}', horizontalalignment='right', fontsize=12)
    axes[i, 1].text(y_max, y_min, f'r={val_corr[i]:.4f}', horizontalalignment='right', fontsize=12)

    axes[i, 0].set_ylabel(y.columns[i], fontsize=16)

plt.savefig(RESULT_DIR / 'pretrain.png')
plt.show()

torch.save(premodel.state_dict(), RESULT_DIR / 'PBA-ml-pretrain_model.pkl')
state_dict = premodel.state_dict()
torch.save(state_dict, RESULT_DIR / 'PBA-ml-pretrain_model_weights.pth')

with open(RESULT_DIR / 'pretrain_norm_x.pkl', 'wb') as f:
    pickle.dump(norm_x, f)
with open(RESULT_DIR / 'pretrain_norm_y.pkl', 'wb') as f:
...
with pd.ExcelWriter(RESULT_DIR / 'pretrain_result_all.xlsx') as writer:
